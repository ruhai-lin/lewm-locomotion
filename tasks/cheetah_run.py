"""Cheetah run task: one-button train + eval + video render.

CLI:
    python -m simulation.lewm.cheetah_run \
...
- AdaptiveCEMPlanner proposes actions via three sources: colored noise +
  ...
  scores via CheetahStateCostModel.
...
# -----------------------------------------------------------------------------
# Cheetah observation layout (dm_control cheetah.run: 17-dim obs)
#   position: 8, velocity: 9
# -----------------------------------------------------------------------------

@dataclass(frozen=True)
class CheetahObsLayout:
    position: slice = slice(0, 8)
    velocity: slice = slice(8, 17)


CHEETAH_OBS = CheetahObsLayout()
...
class CheetahCostConfig:
    ...
    alive_height: float = 0.0


class CheetahStateCostModel(torch.nn.Module):
    """CEM cost over LeWM-predicted future states.

    Score = w_p·forward + w_s·speed_match + w_u·pose_stability
    Cost  = w_e·energy + w_j·jerk − Score

    Cheetah/run exposes no explicit height/upright observation. The cost
    therefore mostly asks for forward velocity, with a light pose penalty to
    discourage wild flips while keeping this task much easier than walker.
    """

    def __init__(self, model: JEPA, cfg: CheetahCostConfig):
        super().__init__()
        self.model = model
        self.cfg = cfg
    ...
    def get_cost(self, info_dict: dict, action_candidates: torch.Tensor) -> torch.Tensor:
        ...
        speed = future[..., CHEETAH_OBS.velocity.start]
        ...
        pose = future[..., CHEETAH_OBS.position]
        pose_stability = torch.exp(-0.05 * pose.square().mean(dim=-1)).mean(dim=-1)

        score = (
            self.cfg.progress_weight * forward
            + self.cfg.speed_weight * speed_match
            + self.cfg.upright_weight * pose_stability
        )
...
# -----------------------------------------------------------------------------
# Cheetah motor primitives
# -----------------------------------------------------------------------------

class AlternatingLegDrive(MotorPrimitive):
    name = "alternating_leg_drive"

    def sample_params(self, rng):
        return {
            "amp": float(rng.uniform(0.25, 0.9)),
            "period": float(rng.uniform(10.0, 28.0)),
    ...
    def generate(self, params, horizon, action_dim, device):
        ...
        actions = torch.full((horizon, action_dim), float(params["bias"]), dtype=torch.float32, device=device)
        if action_dim >= 6:
            actions[:, 0] += -wave
            actions[:, 1] += wave
            actions[:, 2] += 0.5 * wave
...
class RearLegKick(MotorPrimitive):
    name = "rear_leg_kick"

    def sample_params(self, rng):
        return {
            "amp": float(rng.uniform(0.3, 1.0)),
            "duration": int(rng.integers(4, 14)),
            "front_balance": float(rng.uniform(-0.3, 0.3)),
        }
    ...
    def generate(self, params, horizon, action_dim, device):
        ...
        if action_dim >= 6:
            actions[:d, 0] = -params["amp"]
            actions[:d, 1] = params["amp"]
            actions[:d, 2] = 0.5 * params["amp"]
            actions[:d, 3] = params["front_balance"]
            actions[:d, 4] = -params["front_balance"]
        else:
            actions[:d] = params["amp"]
        return actions


class SpineUndulation(MotorPrimitive):
    name = "spine_undulation"
...
def cheetah_primitives() -> list[MotorPrimitive]:
    return [AlternatingLegDrive(), RearLegKick(), SpineUndulation(), BodyRocking()]
...
def compute_cheetah_metrics(
    rewards: np.ndarray, actions: np.ndarray, speeds: np.ndarray, xpos: np.ndarray,
    ...
    if steps == 0:
        return {
            ...
            "progress_m": 0.0, "avg_speed_mps": 0.0, "survival_fraction": 0.0,
            "action_smoothness": 0.0, "grade_label": "0档：完全没跑起来",
            "num_steps": 0,
        }
    ...
    survival_fraction = float(steps / max(max_steps, 1))
    if len(actions) > 1:
    ...
    progress_quality = np.clip(progress_m / 20.0, 0.0, 1.0)
    speed_quality = np.clip(avg_speed_mps / 4.0, 0.0, 1.0)
    reward_quality = np.clip(episode_return / 1000.0, 0.0, 1.0)
    smoothness_quality = np.clip(action_smoothness, 0.0, 1.0)
    task_score = 1000.0 * (
        0.35 * progress_quality
        + 0.30 * speed_quality
        + 0.15 * survival_fraction
...
# -----------------------------------------------------------------------------
# Cheetah policy: state/action history bookkeeping + Adaptive CEM
# -----------------------------------------------------------------------------

class CheetahLeWMPolicy:
    """dm_control-compatible policy wrapping AdaptiveCEMPlanner."""

    def __init__(
        self,
        model: JEPA,
        cost_model: CheetahStateCostModel,
...
@dataclass
class CheetahTaskConfig:
    # I/O
    dataset_name: str = "cheetah_run"
    run_name: str = "cheetah/run"
    seed: int = 42
    device: str = "cpu"
    # Training schedule
    ...
    fall_reset_height: float = 0.0
    fall_reset_grace: int = 0
    ...
    video_path: str = "cheetah_best_episode.mp4"
    video_fps: int = 40
...
def _train_cfg(c: CheetahTaskConfig) -> LeWMTrainConfig:
...
def _cost_cfg(c: CheetahTaskConfig) -> CheetahCostConfig:
    return CheetahCostConfig(
...
def _cem_cfg(c: CheetahTaskConfig) -> AdaptiveCEMConfig:
...
def _model_update(
    ...
    train_cfg: LeWMTrainConfig, cfg: CheetahTaskConfig, scaler,
) -> dict[str, float]:
    model.train()
    batch = _tensor_batch(
...
def _save_checkpoint(
    ...
    cfg: CheetahTaskConfig, action_dim: int, step: int, best_score: float,
    ...
    torch.save(
        {
            ...
            "model_type": "lewm_cheetah_run_v2",
            "global_step": step,
...
def _eval_single_episode(
    policy: CheetahLeWMPolicy, env, max_steps: int, render: bool,
    ...
    metrics = compute_cheetah_metrics(
        rewards=out["rewards"], actions=out["actions"],
        speeds=out["speeds"], xpos=out["xpos"],
...
def _build_eval_policy(model: JEPA, cfg: CheetahTaskConfig, seed: int) -> CheetahLeWMPolicy:
    cost_model = CheetahStateCostModel(model, _cost_cfg(cfg))
    return CheetahLeWMPolicy(
        model=model, cost_model=cost_model, cem_cfg=_cem_cfg(cfg),
        primitives=cheetah_primitives(),
...
def _evaluate_model(
    model: JEPA, cfg: CheetahTaskConfig, seed: int, *,
    ...
    env = make_dm_env("cheetah", "run", seed=seed)
    results = []
    for ep in range(n):
...
def run_training(cfg: CheetahTaskConfig) -> dict[str, Any]:
    ...
    deploy_dir = stable_home / "checkpoints" / "cheetah" / "lewm"
    if run_dir.exists():
        shutil.rmtree(run_dir)
    run_dir.mkdir(parents=True, exist_ok=True)
    metrics_path = run_dir / "metrics.csv"

    env = make_dm_env("cheetah", "run", seed=cfg.seed)
    action_spec = env.action_spec()
    ...
    pbar = tqdm(
        ...
        desc="LeWM-Cheetah", unit="step", dynamic_ncols=True,
    )
    for step in pbar:
        ...
        cur_height = physics_scalar(env.physics, ("torso_height", "height"), default=1.0)
...
def run_final_eval(cfg: CheetahTaskConfig, model: JEPA) -> dict[str, Any]:
    ...
    env = make_dm_env("cheetah", "run", seed=cfg.seed + 999_999)
...
def build_strategy(config: dict[str, Any]) -> StrategyBundle:
    ...
    cfg = CheetahTaskConfig(**{**asdict(base), **overrides})
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train + eval + render cheetah.run via LeWM-MPC.")
    p.add_argument("--dataset-name", default="cheetah_run")
    p.add_argument("--run-name", default="cheetah/run")
    ...
    p.add_argument("--fall-reset-height", type=float, default=0.0)
    p.add_argument("--fall-reset-grace", type=int, default=0)
    ...
    p.add_argument("--video-path", type=str, default="cheetah_best_episode.mp4")
    ...
    # Model architecture
    p.add_argument("--state-dim", type=int, default=17)
    ...
    # Cost
    p.add_argument("--progress-weight", type=float, default=10.0)
    p.add_argument("--speed-weight", type=float, default=2.0)
    p.add_argument("--height-weight", type=float, default=0.0)
    p.add_argument("--upright-weight", type=float, default=0.5)
    p.add_argument("--alive-weight", type=float, default=0.0)
    p.add_argument("--action-energy-weight", type=float, default=0.02)
    p.add_argument("--action-jerk-weight", type=float, default=0.05)
    p.add_argument("--target-speed", type=float, default=4.0)
    p.add_argument("--stand-height", type=float, default=1.0)
    p.add_argument("--alive-height", type=float, default=0.0)
    # CEM
...
def main() -> None:
    ...
    cfg = CheetahTaskConfig(**cli_kwargs)
